...
const NewGroupConfirm = (props) => {

    const {chores,choreOptions,people,group,submit,goBack} = props;
    ...
    let renderPeopleList;
    if(!props.justChores){
        renderPeopleList = people.map((_,i) => {
            return (
                <li key={uid(`cg_peoplelist${people[i]}`, i)} className={i % 2 === 0 ? "bg-white" : "bg-near-white"}>
                    <div className="pa2 b--black-10 ml2 confirm_people_grid" >
                        ...
                        <select onChange={onPermissionsChange} className='permissions mt0 lh-copy tc center' name={people[i]}>
                            <option className='tc' value='true' >Can Add Chores</option>
                            <option className='tc' value='false' >Not Allowed to Add Chores</option>
                        </select>}
                    </div>
                </li>
            )
        ...
        return (
            <li key={uid(`cg_chore${chores[i]}`,i)} className={i % 2 === 0 ? "bg-white pa2" : "bg-near-white pa2"} name={chores[i]}>
                <div  className={choreOptions[chores[i]].description === '' ? "pa1 b--black-10 ml2 confirm_chores_title_grid" : "pa1 b--black-10 ml2 confirm_chores_grid "}>
                    ...
                    {choreOptions[chores[i]].assignment !== 'Randomly' || choreOptions[chores[i]].exempt === 'None' ? null : <div className="exempt mt0 lh-copy tc center pa1 dib">Exemption : <div className="due mt0 lh-copy tc center pa1 b dib">{choreOptions[chores[i]].exempt}</div></div>} 
    ...
    return (
        <div className='center mw6-ns br3 hidden mv4 '>
            {props.justChores ? null : <h1 className="f4 black b mv0 pv2 ph3 tc">Confirm New Group - {group}</h1>}
            <div className={props.justChores ? "pa3" : "pa3 bt b--black-10"} >
                <div className="f6 f5-ns lh-copy measure mv0">
                    <main className="black-80">
                        <div className="measure center">
                            {props.justChores ? null :
                            <fieldset className="ba b--transparent ph0 mh0">
                                <div>
                                    <h4 className=' black  mv0 pv2 ph3 tc'>Emails / Permissions of Group Members</h4>
                                    <ul className="list pl0 ml0 mv0 mt0 center mw6 ba b--light-silver br2 tc shadow-1">
                                        {renderPeopleList}
                                    </ul>
                                </div>
                            </fieldset>}
                            <fieldset className="ba b--transparent ph0 mh0 ">
                                <div>
                                    ...
                                    <ul className="list pl0 ml0 mv0 mt0 mb0 pa0 center  ba b--light-silver br2 shadow-5">
                                        {renderChoreList}
                                    </ul>
                                </div>
                                <div className='tc pa2 mt4'>
